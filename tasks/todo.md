...
## Phase 2 — 健全な定着フック
- [x] 学習カレンダー（草風ヒートマップ。直近12週、日次活動で濃淡）
- [x] 穏やかなストリーク表示（当日未達は猶予・折れても罰しない・通知圧なし）
- [x] コンボ演出強化 / パーティクル（複数隕石改修で実装済み）

## 追加対応: 公式仕様の確認 & 英語化 — 完了
- 公式 jlpt.jp を確認 → **語彙/漢字/文法のリスト・数は非公開**（FAQ明記）。出題構成と各レベル能力記述のみ公開。
  → 「公式の語数」準拠は不可。能力記述(`LEVEL_DESC`)と構成に準拠する方針に。巷の概算値は非公式として扱う。
- 英語話者向けにUIを**全面英語化**（開始説明・ボタン・結果メッセージ）。例文`ex`のみ日本語(学習コンテンツ)。
- カテゴリ「Hiragana」→「Kana」。アプリアイコンの語(語)は意匠として保持。
- 検証: 全レベルで能力記述切替・英語表示・コンソールエラーなし。
- コミット: `c0aded2`(英語化+公式記述) / `b733f5f`(Phase2 ストリーク+ヒートマップ)。origin/main へ push 済み。
